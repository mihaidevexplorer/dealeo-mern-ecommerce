...
const Products: React.FC<ProductsProps> = ({ title, products }) => {
  ...
  const cleanedGroups = useMemo(() => {
    ...
    return groups
      ...
      .map((g) =>
        g.filter((pl: any) => {
          // Minim necesar ca să poți randa produsul + link
          return (
            !!pl &&
  ...
  const hasSlides = cleanedGroups.length > 0;
          ...
          {cleanedGroups.map((group, i) => (
            <SwiperSlide key={i}>
              <div className="flex flex-col gap-3 sm:gap-2 xs:gap-2 2xs:gap-2">
                {group.map((pl: any, j) => {
                  // Imagine fallback (nu mai “pierzi” produsul dacă images[0] lipsește)
                  const img =
                              ...
                              {[...Array(5)].map((_, idx) => (
                                <span
                                  ...
                                  className={`text-xs xs:text-[11px] 2xs:text-[10px] ${
                                    idx < Math.floor(Number(pl.rating) || 0) ? "text-yellow-400" : "text-gray-300"
                                  }`}
                                >
                                  ★
                                </span>
