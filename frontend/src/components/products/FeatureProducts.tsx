...
const FeatureProducts: React.FC<FeatureProductsProps> = ({ products }) => {
  ...
  const handleAddToCart = (productId: string) => {
    ...
    addToCartMutation.mutate({ userId: userInfo.id, productId, quantity: 1 });
  };

  const handleAddToWishlist = (productId: string) => {
    if (!userInfo) return navigate("/login");
    addToWishlistMutation.mutate({ userId: userInfo.id, productId });
  };
  ...
  return (
    <section className="w-full mx-auto px-8 xl:px-6 lg:px-5 md-lg:px-4 md:px-4 sm:px-3 xs:px-2 2xs:px-2 py-10 xl:py-9 lg:py-9 md:py-8 sm:py-8 xs:py-7 2xs:py-6">
      <div className="mx-auto w-full max-w-7xl">
        ...
        {/* TV-safe GRID:
           IMPORTANT: repeat(N, minmax(0,1fr)) + min-w-0 stops “random cards” from breaking columns */}
        <div
          className="
            grid
            [grid-template-columns:repeat(4,minmax(0,1fr))]
            lg:[grid-template-columns:repeat(3,minmax(0,1fr))]
            md-lg:[grid-template-columns:repeat(3,minmax(0,1fr))]
            ...
            2xs:[grid-template-columns:repeat(1,minmax(0,1fr))]
            gap-6 xl:gap-5 md:gap-4 sm:gap-4 xs:gap-3 2xs:gap-3
          "
        >
          {products.map((p) => {
            ...
            return (
              <div
                key={p._id}
                className="
                  group relative bg-white w-full min-w-0
                  rounded-xl xs:rounded-lg overflow-hidden
                  shadow-sm hover:shadow-xl transition-all duration-300
                  hover:-translate-y-1 xs:hover:-translate-y-0.5
                "
              >
                {/* Image */}
                <div className="relative aspect-square w-full min-w-0 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
                  {hasDiscount && (
                  ...
                  <button
                    ...
                    className="
                      ...
                      p-2 xs:p-1.5 bg-white/80 backdrop-blur-sm rounded-full
                      shadow-md hover:bg-white hover:shadow-lg transition-all duration-300
                      disabled:opacity-60
                    "
                  ...
                  <Link to={`/product/details/${p.slug}`} className="block w-full h-full">
                    {/* Stable: image never forces width */}
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3 xs:p-2 bg-gradient-to-t from-black/60 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 2xs:opacity-100 2xs:translate-y-0">
                    <div className="flex justify-center gap-2 xs:gap-1.5 flex-wrap min-w-0">
                      <Link
                        to={`/product/details/${p.slug}`}
                        className="flex items-center gap-1.5 px-3 py-1.5 xs:px-2 xs:py-1 bg-white/95 backdrop-blur-sm text-gray-800 rounded-full hover:bg-white hover:shadow-lg transition-all duration-300 text-xs font-medium min-w-0"
                        title="Quick view"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 xs:px-2 xs:py-1 bg-blue-600/95 backdrop-blur-sm text-white rounded-full hover:bg-blue-700 hover:shadow-lg transition-all duration-300 text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed min-w-0"
                        title={outOfStock ? "Out of stock" : "Add to cart"}
                      >
                        <RiShoppingCartLine className="text-sm shrink-0" />
                        <span className="min-w-0">Add</span>
                      </button>
                    </div>
                  </div>
                ...
                {/* Details */}
                <div className="p-5 xl:p-4 md:p-3 sm:p-3 xs:p-3 2xs:p-3 min-w-0">
                  <Link to={`/product/details/${p.slug}`} className="block mb-2 min-w-0">
                    <h3 className="text-[15px] xl:text-sm md:text-[13px] xs:text-[13px] font-medium text-gray-800 line-clamp-2 hover:text-blue-600 transition-colors min-w-0 break-words">
                      {p.name}
                    </h3>
                  </Link>
                      ...
                      {p.stock > 0 ? (
                        p.stock < 10 ? (
                          <div className="flex items-center gap-1 min-w-0">
                            <div className="w-2 h-2 xs:w-1.5 xs:h-1.5 bg-orange-500 rounded-full animate-pulse shrink-0" />
                            <span className="text-xs xs:text-[11px] text-orange-600 font-medium min-w-0 break-words">
                              Only {p.stock} left
                            </span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1 min-w-0">
                            <div className="w-2 h-2 xs:w-1.5 xs:h-1.5 bg-green-500 rounded-full shrink-0" />
                            <span className="text-xs xs:text-[11px] text-green-600 min-w-0 break-words">
                              In Stock
                            </span>
                          </div>
                        )
                        ...
                        <div className="flex items-center gap-1 min-w-0">
                          <div className="w-2 h-2 xs:w-1.5 xs:h-1.5 bg-red-500 rounded-full shrink-0" />
                          <span className="text-xs xs:text-[11px] text-red-600 font-medium min-w-0 break-words">
                            Out of Stock
                          </span>
                        </div>
                      )}
                  ...
                  {/* IMPORTANT: removed whitespace-nowrap (can break columns on TV) */}
                  <button
                    onClick={() => handleAddToCart(p._id)}
                    disabled={addToCartMutation.isPending || loader || outOfStock}
                  ...
                  >
                    {addToCartMutation.isPending ? (
                      <>
                        <div className="animate-spin rounded-full h-3 w-3 border-2 border-white border-t-transparent shrink-0" />
                        <span className="min-w-0">Adding...</span>
                      </>
                    ) : (
                      <>
                        <RiShoppingCartLine className="text-base shrink-0" />
