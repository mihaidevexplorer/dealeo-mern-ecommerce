...
import { useHomeState, useGetReviews, useSubmitReview, useProductDetails } from '../hooks/useHome';
...
const Reviews: React.FC<ReviewsProps> = ({ product }) => {
  const [parPage] = useState<number>(10);
  const [pageNumber, setPageNumber] = useState<number>(1);
  
  const { userInfo } = useAuthState();
  const { successMessage, reviews, ratingReview, totalReview, clearMessages } = useHomeState();
  
  const [rat, setRat] = useState<number>(0);
  const [re, setRe] = useState<string>('');
  
  const { refetch: refetchReviews } = useGetReviews(product._id, pageNumber);
  const { refetch: refetchProductDetails } = useProductDetails(product.slug);
  const submitReviewMutation = useSubmitReview();

  const review_submit = (e: FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
    ...
    submitReviewMutation.mutate(obj);
  };
  ...
  }, [successMessage, clearMessages, refetchReviews, refetchProductDetails]);

  useEffect(() => {
    if (product._id) {
      refetchReviews();
    }
  }, [pageNumber, product._id, refetchReviews]);
        ...
        <div className='flex gap-2 flex-col py-4'>
          <div className='flex justify-start items-center gap-5'>
            ...
            <div className='w-[200px] h-[14px] bg-slate-200 relative'>
              <div style={{ width: `${Math.floor((100 * (ratingReview[0]?.sum || 0)) / totalReview)}%` }} className='h-full bg-[#Edbb0E] w-[60%]'>
              </div>
            </div>
          ...
          <div className='flex justify-start items-center gap-5'>
            ...
            <div className='w-[200px] h-[14px] bg-slate-200 relative'>
              <div style={{ width: `${Math.floor((100 * (ratingReview[1]?.sum || 0)) / totalReview)}%` }} className='h-full bg-[#Edbb0E] w-[70%]'>
              </div>
            </div>
          ...
          <div className='flex justify-start items-center gap-5'>
            ...
            <div className='w-[200px] h-[14px] bg-slate-200 relative'>
              <div style={{ width: `${Math.floor((100 * (ratingReview[2]?.sum || 0)) / totalReview)}%` }} className='h-full bg-[#Edbb0E] w-[40%]'>
              </div>
            </div>
          ...
          <div className='flex justify-start items-center gap-5'>
            ...
            <div className='w-[200px] h-[14px] bg-slate-200 relative'>
              <div style={{ width: `${Math.floor((100 * (ratingReview[3]?.sum || 0)) / totalReview)}%` }} className='h-full bg-[#Edbb0E] w-[30%]'>
              </div>
            </div>
          ...
          <div className='flex justify-start items-center gap-5'>
            ...
            <div className='w-[200px] h-[14px] bg-slate-200 relative'>
              <div style={{ width: `${Math.floor((100 * (ratingReview[4]?.sum || 0)) / totalReview)}%` }} className='h-full bg-[#Edbb0E] w-[10%]'>
              </div>
            </div>
...
export default Reviews;//modificat