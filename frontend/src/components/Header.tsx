...
const Header: React.FC = () => {
   ...
   * - Acestea trebuie apelate TOP-LEVEL (NU în useEffect).
   * - În hook-uri, tu trebuie să ai `enabled: !!userId` ca să nu facă fetch când userId e gol.
   */
  ...
  return (
    <div className="w-full bg-white">
          ...
          <div className="flex w-full justify-between items-center h-[45px] text-white">
            ...
            <div className="flex justify-center items-center gap-10">
              ...
              <div className="flex group cursor-pointer text-white text-sm justify-center items-center gap-1 relative after:h-[18px] after:w-[1px] after:bg-white/30 after:-right-[16px] after:absolute before:absolute before:h-[18px] before:bg-white/30 before:w-[1px] before:-left-[20px]">
                ...
                {/* language dropdown - doar opacity/position (nu transition-all pe layout critic) */}
                ...
                <ul className="flex justify-start items-start gap-8 text-sm font-semibold md-lg:hidden">
                  <li>
                    <Link
                      ...
                      className={`p-2 block transition-colors ${
                        pathname === "/" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"
                      }`}
                    >
                      HOME
                    </Link>
                  ...
                  <li>
                    <Link
                      to="/shops"
                      className={`p-2 block transition-colors ${
                        pathname === "/shops" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"
                      }`}
                    >
                      SHOP
                    </Link>
                  ...
                  <li>
                    <Link
                      to="#"
                      className={`p-2 block transition-colors ${
                        pathname === "/blog" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"
                      }`}
                    >
                      BLOG
                    </Link>
                  ...
                  <li>
                    <Link
                      to="#"
                      className={`p-2 block transition-colors ${
                        pathname === "/about" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"
                      }`}
                    >
                      ABOUT US
                    </Link>
                  ...
                  <li>
                    <Link
                      to="#"
                      className={`p-2 block transition-colors ${
                        pathname === "/contact" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"
                      }`}
                    >
                      CONTACT
                    </Link>
      ...
      {/* MOBILE SIDEBAR (FIXED + TRANSFORM) */}
      <div className="hidden md-lg:block">
        {/* Overlay: fixed inset-0, opacity + pointer-events (nu visible/invisible) */}
        <div
          onClick={() => setShowShidebar(true)}
          className={`fixed inset-0 bg-[rgba(0,0,0,0.5)] z-40 md-lg:block hidden transition-opacity duration-200 ${
            showShidebar ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
        />

        {/* Panel: fixed + translateX (nu left) */}
        <div
          className={`fixed top-0 left-0 w-[300px] h-screen bg-white z-50 shadow-2xl md-lg:block hidden overflow-y-auto py-6 px-8 transform transition-transform duration-200 ${
        ...
        >
          ...
          <div className="flex justify-start items-center gap-6 mb-8">
            ...
            {userInfo ? (
              <Link className="flex cursor-pointer justify-center items-center gap-2 text-sm text-gray-700" to="/dashboard" onClick={() => setShowShidebar(true)}>
                <FaUser className="w-4 h-4" />
                <span>{userInfo.name}</span>
              </Link>
            ) : (
              <Link className="flex cursor-pointer justify-center items-center gap-2 text-sm text-gray-700" to="/login" onClick={() => setShowShidebar(true)}>
                <FaLock className="w-4 h-4" />
                <span>Login</span>
              </Link>
            )}
          ...
          <ul className="flex flex-col justify-start items-start text-sm font-semibold mb-8">
            <li className="w-full">
              <Link to="/" onClick={() => setShowShidebar(true)} className={`py-3 block transition-colors ${pathname === "/" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"}`}>
                HOME
              </Link>
            </li>
            <li className="w-full">
              <Link to="/shops" onClick={() => setShowShidebar(true)} className={`py-3 block transition-colors ${pathname === "/shops" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"}`}>
                SHOP
              </Link>
            </li>
            <li className="w-full">
              <Link to="#" onClick={() => setShowShidebar(true)} className={`py-3 block transition-colors ${pathname === "/blog" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"}`}>
                BLOG
              </Link>
            </li>
            <li className="w-full">
              <Link to="#" onClick={() => setShowShidebar(true)} className={`py-3 block transition-colors ${pathname === "/about" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"}`}>
                ABOUT US
              </Link>
            </li>
            <li className="w-full">
              <Link to="#" onClick={() => setShowShidebar(true)} className={`py-3 block transition-colors ${pathname === "/contact" ? "text-orange-500" : "text-gray-700 hover:text-orange-500"}`}>
                CONTACT
              </Link>
            </li>
          </ul>
            ...
            <div className="bg-white relative">
              ...
              <button
                ...
                className={`h-[50px] w-full ${
                  categoryShow ? "bg-gradient-to-r from-orange-500 to-orange-600" : "bg-gradient-to-r from-orange-400 to-orange-500"
                } text-white flex justify-between items-center px-5 gap-3 font-semibold cursor-pointer rounded-lg shadow-md hover:shadow-lg transition-shadow transition-colors duration-300`}
              ...
              <div
                className={`absolute left-0 top-full z-[99999] bg-white w-full rounded-b-lg shadow-xl transition-opacity transition-transform duration-200 ${
              ...
              <div className="w-8/12 md-lg:w-full">
                <div className="flex bg-gray-50 rounded-lg h-[50px] items-center shadow-md overflow-hidden border border-gray-200 focus-within:border-orange-400 transition-colors">
                  ...
                  <button
                    type="button"
                    onClick={search}
                    className="h-full px-8 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold uppercase text-sm transition-colors duration-300 hover:from-orange-600 hover:to-orange-700"
                  >
                    Search
                  </button>
                </div>
