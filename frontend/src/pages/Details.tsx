...
const Details = () => {
    ...
    // Get visible thumbnails for manual mode (currently unused but may be needed for fallback)
    // const getVisibleThumbnails = () => {
    //     if (!product?.images) return [];
    //     return product.images.slice(thumbnailStartIndex, thumbnailStartIndex + visibleThumbnails);
    // };
...
const canNavigateNext = thumbnailSwiperRef.current ? !thumbnailSwiperRef.current.isEnd : (product?.images ? thumbnailStartIndex + visibleThumbnails < product.images.length : false);
                ...
                <div className='space-y-4'>
                    ...
                    <div className='relative group bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300'>
                        <div className='aspect-square relative overflow-hidden'>
                            ...
                            {/* Premium Expand/Zoom Icon */}
                            <button 
                                onClick={openImageModal}
                                className='absolute top-4 right-4 bg-white/90 backdrop-blur-sm hover:bg-white text-gray-700 hover:text-orange-500 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0'
                            >
                                <FaExpand size={18} />
                            </button>
                            ...
                            {product?.images && product.images.length > 1 && (
                                <>
                                    <button 
                                        onClick={prevImage}
                                        className='absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/90 backdrop-blur-sm hover:bg-white text-gray-700 hover:text-orange-500 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 z-10'
                                    >
                                        <HiChevronLeft size={24} />
                                    </button>
                                    <button 
                                        onClick={nextImage}
                                        className='absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/90 backdrop-blur-sm hover:bg-white text-gray-700 hover:text-orange-500 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 z-10'
                                    >
                    ...
                    {/* Enhanced Thumbnail Gallery with Fallback */}
                    {product?.images && product.images.length > 1 && (
                        <div ref={thumbnailContainerRef} className='relative bg-white rounded-lg p-2 shadow-sm border border-gray-100'>
                           ...
                           {/* Enhanced Navigation Buttons - Show for 4+ images */}
                            {product.images.length > 4 && (
                                <>
                                    <button 
                                        onClick={() => handleThumbnailNavigation('prev')}
                                        disabled={!canNavigatePrev}
                                        className={`absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-3 bg-white shadow-xl rounded-full p-2 z-30 transition-all duration-300 border border-gray-200 ${
                                            canNavigatePrev 
                                    ...
                                    <button 
                                        ...
                                        className={`absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-3 bg-white shadow-xl rounded-full p-2 z-30 transition-all duration-300 border border-gray-200 ${
                                            canNavigateNext 
                                                ? 'hover:bg-orange-50 hover:text-orange-500 hover:scale-110 cursor-pointer' 
                                                : 'opacity-50 cursor-not-allowed'
                                        }`}
                    ...
                    <div className='fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm'>
                        <div className='relative max-w-6xl max-h-full'>
                            ...
                            <button 
                                ...
                                className='absolute top-4 right-4 text-white hover:text-orange-400 z-20 bg-black/50 rounded-full p-2 hover:bg-black/70 transition-all duration-300'
                            >
                                <IoClose size={32} />
                            </button>
                            ...
                            {product?.images && product.images.length > 1 && (
                                <>
                                    <button 
                                        onClick={prevImage}
                                        className='absolute left-4 top-1/2 transform -translate-y-1/2 text-white hover:text-orange-400 bg-black/50 rounded-full p-4 hover:bg-black/70 transition-all duration-300 hover:scale-110'
                                    >
                                        <HiChevronLeft size={40} />
                                    </button>
                                    <button 
                                        onClick={nextImage}
                                        className='absolute right-4 top-1/2 transform -translate-y-1/2 text-white hover:text-orange-400 bg-black/50 rounded-full p-4 hover:bg-black/70 transition-all duration-300 hover:scale-110'
                                    >
                            ...
                            {/* Modal Image Counter */}
                            {product?.images && product.images.length > 1 && (
                                <div className='absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-black/70 backdrop-blur-sm text-white px-6 py-3 rounded-full text-lg font-medium border border-white/20'>
        ...
        relatedProducts.map((p, i) => {
            return (
                ...
                <SwiperSlide key={i}>
                    ...
                    <div className='absolute h-full w-full top-0 left-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300'> 
                    </div>
                           </div>
            {
            p.discount !== 0 && <div className='flex justify-center items-center absolute text-white w-[38px] h-[38px] rounded-full bg-red-500 font-semibold text-xs left-2 top-2'>{p.discount}%
            ...
            <h2 className='text-slate-600 text-lg font-bold group-hover:text-orange-500 transition-colors duration-300'>{p.name} </h2>
    ...
    {relatedProducts.length > 3 && (
        <>
            <button 
                ...
                className='absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-6 bg-white shadow-lg rounded-full p-3 z-10 hover:bg-gray-50 transition-colors duration-200'
            >
                <HiChevronLeft size={24} className='text-gray-700' />
            </button>
            <button 
                onClick={handleRelatedNext}
                className='absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-6 bg-white shadow-lg rounded-full p-3 z-10 hover:bg-gray-50 transition-colors duration-200'
            >
