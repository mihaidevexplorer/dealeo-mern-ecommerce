...
// Interface for location state - exact ca în JSX
...
const Shipping = () => {
    const { state } = useLocation() as { state: ShippingLocationState };
    const placeOrderMutation = usePlaceOrder();
    const navigate = useNavigate();
    const { userInfo } = useAuthState();
    
    const [res, setRes] = useState(false);
    const [shippingState, setState] = useState({
        name: '',
        address: '',
        phone: '',
        post: '',
    ...
    const placeOrder = () => {
        ...
        // Transform products pentru API conform structurii din backend (PlaceOrderBody)
        const transformedProducts = products.map(shop => {
            const sellerProducts = shop.products
                .filter(item => item.productInfo?._id)
                .map(item => ({
                    _id: item.productInfo._id!, // Folosim ID-ul real al produsului, nu cu prefix "cart_"
                    ...
                    productInfo: {
                        ...
                        stock: 100, // Valoare default pentru stock
                        discount: Number(item.productInfo.discount) || 0,
                        sellerId: '66e9938219376cf9261704ba', // Folosim un ObjectId valid temporar
        ...
        // Calculează totalul comenzii și numărul de items
        const totalProductPrice = transformedProducts.reduce((total, seller) => 
            total + seller.price, 0
        );

        const totalItems = transformedProducts.reduce((total, seller) => 
            total + seller.products.reduce((sellerTotal, product) => 
                sellerTotal + product.quantity, 0
            ), 0
        );
        ...
        // Validează shipping info
        const requiredFields = ['name', 'address', 'phone', 'post', 'province', 'city', 'area'];
        const missingFields = requiredFields.filter(field => 
            !shippingState[field as keyof typeof shippingState]?.trim()
        );
        ...
        // Prepare order data conform interfetei din backend
        placeOrderMutation.mutate({
            ...
            items: totalItems, // Calculat corect din produse
            navigate // Pentru hook
        });
    };
                            ...
                            {res && (
                                <div className='flex flex-col gap-1'>
                                    ...
                                    <p>
                                        ...
                                        <span>{shippingState.phone} {shippingState.address} {shippingState.province} {shippingState.city} {shippingState.area}</span>
