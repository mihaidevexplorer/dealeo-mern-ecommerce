...
const Pagination = ({pageNumber,setPageNumber,totalItem,parPage,showItem}) => {
    ...
    const createBtn = () => {
        ...
        for (let i = startPage; i < endPage; i++) {
            btns.push(
                <li onClick={()=>setPageNumber(i)} className={` ${pageNumber === i ? 'bg-indigo-300 shadow-lg shadow-indigo-300/50 text-white' : 'bg-gray-600 hover:bg-blue-400 shadow-lg hover:shadow-blue-500/50 hover:text-white text-[#d0d2d6]'} w-[33px] h-[33px] rounded-full flex justify-center items-center cursor-pointer `}>
                    {i}                    
                </li>
            ) 
        }
